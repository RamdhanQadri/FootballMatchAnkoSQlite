...
import rqk.fmas.R
import rqk.fmas.R.color.colorAccent
import rqk.fmas.R.drawable.ic_add_to_favorites
import rqk.fmas.R.drawable.ic_added_to_favorites
import rqk.fmas.R.id.add_to_favorite
import rqk.fmas.R.menu.detail_menu
...
import rqk.fmas.utils.formatDate
...
class DetailActivity : AppCompatActivity(), MainView {
    ...
    private val presenterDetail: MainPresenter = MainPresenter(this, ApiRepository(), Gson())
            ...
            val result = select(Favorite.TABLE_FAVORITE)
                .whereArgs(
                    "(EVENT_ID = {match})", "match" to matchEvent +
                            "(HOME_TEAM_ID = {home})", "home" to homeEvent +
                            "(AWAY_TEAM_ID = {away})", "away" to awayEvent
                )
    ...
    private fun addEventFavorites() {
        try {
            ...
            snackbar(swipeRefreshLayout, R.string.added).show()
    ...
    private fun removeFromFavorites() {
        try {
            ...
            snackbar(swipeRefreshLayout, R.string.removed).show()
        } catch (e: SQLiteConstraintException) {
            snackbar(swipeRefreshLayout, e.localizedMessage).show()
        }
    }
